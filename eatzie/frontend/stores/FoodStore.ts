...
interface FoodState {
  ...
  addFood: (data: FoodModel, restaurantId: number) => Promise<FoodItemD | null>;
}
...
export const useFoodStore = create<FoodState>()(
  subscribeWithSelector((set, get) => ({
    ...
    addFood: async (data: FoodModel, restaurantId: number) => {
      try {
        set((state) => ({
          loading: { ...state.loading, [restaurantId]: true },
          error: { ...state.error, [restaurantId]: null },
        ...
        const response = await SearchService.createFood(data, restaurantId);

        if (!response || !response.data) {
          throw new Error("Không nhận được dữ liệu món ăn từ API");
        }

        const newFood = response.data;

        set((state) => ({
          foods: { ...state.foods, [newFood.id]: newFood },
          foodsByRestaurant: {
            ...state.foodsByRestaurant,
            [restaurantId]: [
              ...(state.foodsByRestaurant[restaurantId] || []),
              newFood,
            ],
          },
          lastAddedFoodId: newFood.id,
        }));
        ...
        set((state) => ({
          error: {
            ...state.error,
            [restaurantId]: err?.message || "Không thể tạo món ăn",
          },
        }));
        return null;
      } finally {
