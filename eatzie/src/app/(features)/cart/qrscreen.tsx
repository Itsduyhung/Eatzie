...
export default function QrScreen() {
  const { total, orderId, paymentCode, paymentLink, qrCode } = useLocalSearchParams();
  ...
  const qrCodeString = Array.isArray(qrCode) ? qrCode[0] : (qrCode as string | undefined);
  ...
  const isUrl = qrCodeString?.startsWith("http://") || qrCodeString?.startsWith("https://");
  const isRawQRData = qrCodeString && !isBase64Image && !isUrl;
  
  // Debug: Log qrCode để kiểm tra
  useEffect(() => {
    if (qrCodeString) {
      console.log("📱 QR Code received:", qrCodeString.substring(0, 50) + "...");
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        const orderResponse = await OrderService.getOrderDetail(Number(orderId));
        ...
        if (orderData) {
          ...
          if (orderData.items && orderData.items.length > 0) {
            const itemsDescription = orderData.items
              .map((item: any) => `${item.foodName || `Món ${item.foodId}`} x${item.quantity}`)
              .join(", ");
  ...
  const checkPaymentStatus = async () => {
    ...
    try {
      ...
      if (orderData && orderData.status) {
        ...
        if (status === "Đã thanh toán" || status === "PAID" || status === "paid") {
      ...
      if (orderId) {
        try {
          const paymentResponse = await PaymentService.verifyPaymentByOrder(Number(orderId));
          
          // Handle both wrapped and unwrapped responses
          const paymentData = (paymentResponse as any).data || paymentResponse;
          
          if (paymentData?.status === "PAID") {
            console.log("✅ Payment verified via PaymentService.verifyPaymentByOrder!");
            setIsPolling(false);
          ...
          console.log("Payment verification failed, continuing with order status check");
        }
      }
    } catch (error) {
    ...
    <ScrollScreenLayout headerLeftIcons={[<BackButton key="back" />]}>
      <YStack flex={1} gap="$4" paddingHorizontal="$4" paddingBottom="$4">
        ...
        <YStack flex={1} justifyContent="center" alignItems="center" gap="$3">
          ...
          <YStack alignItems="center" gap="$2" padding="$4" backgroundColor="white" borderRadius={16}>
            {qrCodeString && qrCodeString.trim() !== "" ? (
              ...
              isBase64Image || isUrl ? (
                ...
                <Image
                  ...
                  resizeMode="contain"
                  onError={(error) => {
                    console.error("Error loading QR code image from PayOS:", error);
                  }}
                />
              ) : isRawQRData ? (
              ...
              <View style={{ width: 220, height: 220, backgroundColor: "#f0f0f0", justifyContent: "center", alignItems: "center", borderRadius: 8 }}>
          ...
          {/* Loading indicator when checking payment */}
          {isPolling && (
