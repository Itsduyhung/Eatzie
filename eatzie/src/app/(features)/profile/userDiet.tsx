...
import { YStack, XStack, Text, Button, Input, TextArea, Select, Adapt, Sheet } from "tamagui";
...
import { UserDiet, DietType, DietTypeLabels } from "@/types/userDiet.types";
...
const UserDietScreen = () => {
  ...
  const [editMinSpending, setEditMinSpending] = useState<number | undefined>(undefined);
  const [editMaxSpending, setEditMaxSpending] = useState<number | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    if (user?.userId) {
      console.log("🔍 UserDietScreen: Calling ProfileService.getProfile for userId:", user.userId);
      ProfileService.getProfile(Number(user.userId)).then((profileData) => {
        console.log("✅ UserDietScreen: ProfileService.getProfile returned:", profileData);
  ...
  useEffect(() => {
    if (userDiet) {
      ...
      setEditDietType(DietTypeLabels[userDiet.diet_type as keyof typeof DietTypeLabels] || "");
  ...
  const group1: ItemDef[] = [
    {
      ...
      value: userDiet ? DietTypeLabels[userDiet.diet_type as DietType] || "Chưa chọn" : "Chưa có thông tin",
      editable: true,
      onPress: () => {
  ...
  const group2: ItemDef[] = [
    ...
    },
    {
      ...
      value: userDiet?.min_spending && userDiet?.max_spending
  ...
  return (
    ...
    >
      <YStack gap="$1" padding="$2">
        ...
        >
          <Text color="#fff" fontSize="$5" fontWeight="bold">Cập nhật hồ sơ vị giác</Text>
        </CustomButton>
        <Sheet
          open={editOpen}
          ...
          modal={false}
        >
          <YStack position="absolute" top={-150} left={0} right={-10} bottom={0} zIndex={999} backgroundColor="rgba(0,0,0,0.15)" justifyContent="flex-start" alignItems="center">
            <YStack width="100%" padding="$5" backgroundColor="#fff" borderRadius={0} shadowColor="#000" shadowOpacity={0.15} shadowRadius={12} marginTop={40}>
              <XStack alignItems="center" marginBottom="$4">
                ...
                >
                  <ChevronLeft size={24} color="#050505ff" style={{ transform: [{ rotate: '180deg' }] }} />
                </CustomButton>
                <Text fontSize="$6" fontWeight="bold" textAlign="center" flex={1} color="#000">Chỉnh sửa hồ sơ vị giác</Text>
              </XStack>
              <YStack gap="$3">
                <Text fontSize="$5" marginBottom={2} color="#000">Loại chế độ ăn</Text>
                <CustomButton
                ...
                >
                  <XStack width="100%" alignItems="center" justifyContent="space-between">
                    <Text color="#000" fontSize="$5" textAlign="left">{editDietType || "Chọn loại chế độ ăn"}</Text>
                    <ChevronDown size={20} color="#999" />
                  </XStack>
                </CustomButton>
                {showDietTypeDropdown && (
                  <YStack backgroundColor="#F5F5F5" borderRadius={8} shadowColor="#000" shadowOpacity={0.1} shadowRadius={8} marginTop={4}>
                    {["Ăn chay", "Ăn mặn", "Ăn cả chay lẫn mặn"].map((label) => (
                      <CustomButton
                        key={label}
                        backgroundColor="#F5F5F5"
                        paddingVertical={12}
  ...
  const [showDietTypeDropdown, setShowDietTypeDropdown] = useState(false);
                <Text fontSize="$5" marginBottom={2} color="#000">Thức ăn dị ứng</Text>
                <Input
                  ...
                  color="#000"
                />
                <Text fontSize="$5" marginBottom={2} color="#000">Thức ăn yêu thích</Text>
                <Input
                  ...
                  color="#000"
                />
                <Text fontSize="$5" marginBottom={2} color="#000">Khoảng chi tiêu tối thiểu (VNĐ)</Text>
                <Input
                  ...
                  color="#000"
                />
                <Text fontSize="$5" marginBottom={2} color="#000">Khoảng chi tiêu tối đa (VNĐ)</Text>
                <Input
                  keyboardType="numeric"
                ...
                <CustomButton
                  ...
                  onPress={async () => {
                    try {
                      ...
                      else if (editDietType === "Ăn cả chay lẫn mặn") dietTypeNumber = 2;
                      else {
                        Alert.alert("Lỗi", "Loại chế độ ăn không hợp lệ. Vui lòng chọn đúng tên chế độ ăn.");
                        return;
                      }
                      const updateData = {
                      ...
                      const res = await UserDietService.updateUserDiet(updateData);
                      if (res.isSuccess) {
                        Alert.alert("Thành công", "Cập nhật hồ sơ vị giác thành công!");
                        setEditOpen(false);
                      ...
                      Alert.alert("Lỗi", "Có lỗi xảy ra khi cập nhật hồ sơ vị giác.");
                    }
                  }}
                >
                  <Text color="#fff" fontSize="$5" fontWeight="bold">Lưu thay đổi</Text>
                </CustomButton>
              </YStack>
            </YStack>
          </YStack>
